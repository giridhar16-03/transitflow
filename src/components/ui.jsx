...
export function SectionTitle({ eyebrow, title, body, align = "left" }) {
  return (
    <div className={cn(align === "center" && "mx-auto text-center", "max-w-3xl") }>
      ...
      <h2 className="mt-3 font-display text-3xl tracking-tight text-foreground sm:text-4xl md:text-5xl">{title}</h2>
      {body ? <p className="mt-4 text-sm leading-7 text-muted-foreground sm:text-base md:text-lg">{body}</p> : null}
    </div>
  );
}
